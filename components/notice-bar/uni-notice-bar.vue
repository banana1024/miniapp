	...
	<view v-if="show" :style="{backgroundColor:backgroundColor,color:color}" class="uni-noticebar" @click="onClick">
		...
		<view :class="{'uni-noticebar--flex': scrollable || single || moreText}" class="uni-noticebar__content">
			...
			<view :class="{'uni-noticebar--scrollable':scrollable,'uni-noticebar--single':!scrollable && (single || moreText)}" class="uni-noticebar__content-text">
				<view :id="elId" :style="{'animation': animation,'-webkit-animation': animation}" class="uni-noticebar__content-inner">
					<block v-for="item in text" :key="item.id">
						<img :src="item.headImg" alt="" style="width:30px;height:30px;vertical-align: middle;border-radius: 50%;" mode="widthFix">\t\t
					
						<view style="margin-right: 30px;font-size: 24rpx;display: inline-block;"> 	{{item.nickname}}{{ item.message }}\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t </view>
